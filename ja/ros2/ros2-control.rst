...
現在基本的な機能が実装されていて、ROS 2 Humble（Ubuntu 22.04）上で利用することが可能です。
ここではサンプルを動かすまでの手順を紹介します。

.. note:: ROS 2 Jazzy (Ubuntu 24.04）についても対応を進めていますが、ros2_controlのAPIがHumbleから大きく変わっていることもあり、現状ではうまく動作させることができていません。

.. note:: 以前はこの機能はfeature/ros2-control-betaという別ブランチで公開していましたが、2025年10月17日よりmasterで利用可能としています。
...
ROS2プラグインが提供する「ROS2Controlアイテム」によって、Choreonoid上でros2_controlを利用できるようになります。
...
ros2_controlを用いてChoreonoid上の仮想ロボットを制御するサンプルを :doc:`ros2-mobile-robot-tutorial` に導入しています。

.. note:: 以前は :doc:`ros2-mobile-robot-tutorial` のリポジトリの "ros2-control" ブランチで公開していましたが、2025年10月17日よりmainブランチで利用可能としています。

サンプルのビルドと実行は基本的に :doc:`ros2-mobile-robot-tutorial` と同様の手順で進めることになりますので、そちらの解説スライドも参照ください。以下ではサンプルを実行するまでの手順を簡単に説明します。ワークスペース上のChoreonoid本体とchoreonoid_rosのパッケージは最新の状態になっているものとします。
...
このサンプルは :doc:`ros2-mobile-robot-tutorial` に沿ったものとしており、モバイルロボットの車体は "/base_controller/cmd_vel" トピックを介して制御するようになっています。launchファイルで起動されるrqt_robot_steeringのインタフェースでこれを行えるようにしてあります。

ただし "/base_controller/cmd_vel" トピックについて、タイムスタンプが付与される "TwistStamped" 型のメッセージである必要があります。このため、下図のようにrqt_robot_steeringの "stamped" のチェックを入れて、このメッセージ型になるようにしてください。
