...
* シーンバーの「編集モードへの切り替え」ボタンを押す。これがOFFの時は閲覧モードで、ONにすると編集モードとなる。
* シーンビューにフォーカスがある状態で、ESCキーを押す。（押す度にモードが切り替わります。）
* シーンビュー上でダブルクリックする。（ただし編集モードの時にはダブルクリックが他の操作に割り当てられる場合もあります。）

現在のモードは、シーンバーの「編集モードへの切り替え」ボタンが押し込まれているかどうかで分かります。また、編集モードの時にはモデルに編集用のマーカー等が表示される場合があり、そのような表示があるかどうかで判別することもできます。
...
シーンバーの「シーン全体に視点をフィット」ボタンを押すと、視点の方向は変えずに、シーン上の全ての物体が見えるように並進位置とズームを調節します。これは視点変更操作を行っている間にどこを見ているか分からなくなった場合に有用です。
...
シーンバーの「ワイヤフレーム描画」ボタンをONにすると、シーンがワイヤフレームで描画されるようになります。これはモデルのポリゴン構成を見たい場合や、物体の重なり具体を見たい場合などに便利です。シーンの描画方法を変更する要素は他にもいくつかあり、以下で説明する設定ダイアログで設定することが可能です。
...
シーンビューの描画方法や挙動については他にも設定可能な項目があり、 :ref:`basics_sceneview_scenebar` の「設定ダイアログ表示」ボタンを押すと表示されるダイアログで細かく設定することが可能です。このダイアログは「カメラ」「ライティング」「背景」「描画」の4つのパネルと、その下にあるオプション領域で構成されています。各パネルからアクセス可能な主な設定項目の概要を以下に示します。

カメラパネル
~~~~~~~~~~~~

.. 英訳指示： 見出し「カメラパネル」は "Camera panel" としてください。
.. 英訳指示： 表中の項目名は次の対応で英訳してください。視野角→Field of view、方向→Direction、自動→Auto、縦→Vertical、横→Horizontal、近方→Near、遠方→Far、全カメラの遠方を無限遠にする（OpenGL 4.5以降）→Use infinite far clip distance for all cameras (OpenGL 4.5 or later)、カメラロールを制限→Restrict camera roll、垂直軸→Vertical axis、上下反転→Upside down。
 ...
 * - 方向
   - 視野角の基準とする方向を「自動」「縦」「横」から選択します。「自動」ではビューのアスペクト比に応じて自動決定されます。
 * - 近方／遠方
   - 視点から見た描画範囲（クリッピング距離）を設定します。描画に問題がなければ特に指定する必要はありません。
 * - 全カメラの遠方を無限遠にする（OpenGL 4.5以降）
   - すべてのカメラの遠方クリッピング距離を実質的に無限遠として扱います。OpenGL 4.5以降の環境で有効です。
 * - カメラロールを制限
   - 視点回転時のロール（光軸回りの回転）を抑制し、画面の上下方向が崩れないようにします。
 * - 垂直軸
   - 鉛直上向きとみなす軸をX、Y、Zから選択します。Choreonoidでは通常Zを上向きとして使います。
 * - 上下反転
   - シーンビューの画像を上下反転して表示します。

ライティングパネル
~~~~~~~~~~~~~~~~~~

.. 英訳指示： 見出し「ライティングパネル」は "Lighting panel" としてください。
.. 英訳指示： 表中の項目名は次の対応で英訳してください。ライティングモード→Lighting mode、標準→Normal、最小限→Minimum、ソリッドカラー→Solid color、スムーズシェーディング→Smooth shading、裏面除去→Back face culling、有効→Enabled、無効→Disabled、強制→Forced、ワールドライト→World light、ヘッドライト→Head light、環境光→Ambient light、照度→Intensity、影→Shadow、マテリアル環境光を正規化→Normalize material ambient、追加のライト→Additional lights、影：ライト→Shadow for light、テクスチャ→Texture、フォグ→Fog、影のアンチエイリアシング→Anti-aliasing of shadows。
 ...
 * - 項目
   - 内容
 * - ライティングモード
   - 描画に用いるライティング処理を「標準」「最小限」「ソリッドカラー」から選択します。「最小限」は軽量のライティング、「ソリッドカラー」はマテリアルの色のみで描画するモードです。
 * - スムーズシェーディング
   - スムーズシェーディングのON/OFFを切り替えます。OFFにするとフラットシェーディングになります。
 * - 裏面除去
   - ポリゴン裏面のカリングを「有効」「無効」「強制」から選択します。「強制」はモデル側の指定によらず常にカリングを行います。
 * - ワールドライト
   - シーン上に固定されたライト（通常上方から照射される）のON/OFFを切り替えます。「照度」で明るさを、「影」でこのライトによる影の生成を設定できます。
 * - ヘッドライト
   - 常に視点位置から照射されるライトのON/OFFを切り替えます。「照度」で明るさを設定できます。
 * - 環境光
   - 環境光（アンビエントライト）のON/OFFを切り替えます。「照度」で強さを設定できます。「マテリアル環境光を正規化」をONにすると、各マテリアルの環境光成分を一様化して描画します。
 * - 追加のライト
   - シーン上に読みこまれたモデルがライトを有する場合、そのON/OFFを切り替えます。「影：ライト」のチェックをONにすると、指定した番号のライトによる影が描画されます（最大2つまで）。
 * - テクスチャ
   - テクスチャ表示のON/OFFを切り替えます。
 * - フォグ
   - シーン中に定義されたフォグ（霧）効果のON/OFFを切り替えます。
 * - 影のアンチエイリアシング
   - 影の輪郭にアンチエイリアシング処理を施します。

背景パネル
~~~~~~~~~~

.. 英訳指示： 見出し「背景パネル」は "Background panel" としてください。
.. 英訳指示： 表中の項目名は次の対応で英訳してください。背景色→Background color、座標軸→Coordinate axes、XY (床) グリッド→XY (Floor) Grid、XZグリッド→XZ Grid、YZグリッド→YZ Grid、長さ→Span、間隔→Interval、色→Color。
 ...
 * - XY (床) グリッド／XZグリッド／YZグリッド
   - それぞれの平面に対応するグリッド線の表示ON/OFFと、グリッド全体の「長さ」（範囲の大きさ）、線の「間隔」、線の「色」を設定します。

描画パネル
~~~~~~~~~~

.. 英訳指示： 見出し「描画パネル」は "Drawing panel" としてください。
.. 英訳指示： 表中の項目名は次の対応で英訳してください。デフォルト色→Default color、デフォルト線幅→Default line width、デフォルト点サイズ→Default point size、干渉ハイライト色→Collision highlight color、干渉線色→Collision line color、アンチエイリアシング（MSAA）→Anti-Aliasing (MSAA)、システム設定→System Default、オフ→Off、法線の表示→Normal Visualization、軽量視点変更→Lightweight view change、FPSテスト→FPS Test。
.. 英訳指示： 「メインメニューの「オプション」-「OpenGL」-「MSAAレベル（システム設定）」」は "the main menu "Options" - "OpenGL" - "MSAA Level (System Default)"" としてください。
.. 英訳指示： 軽量視点変更の説明で言及している「シーンアイテムのプロパティ「軽量レンダリング」」のプロパティ名は "Lightweight rendering" としてください。
 ...
 * - 干渉ハイライト色
   - モデル間の干渉箇所をハイライト表示する際の色を設定します。
 * - 干渉線色
   - 干渉線の色を設定します。
 * - アンチエイリアシング（MSAA）
   - マルチサンプルアンチエイリアシングのレベルを「システム設定」「オフ」「2x」「4x」「8x」「16x」から選択します。「システム設定」を選んだ場合は、メインメニューの「オプション」-「OpenGL」-「MSAAレベル（システム設定）」で設定したレベルが用いられます。
 * - 法線の表示
   - ポリゴンの各点の法線を表示します。法線の長さも設定できます。
 * - 軽量視点変更
   - 視点をマウスで操作している間、描画を一時的に簡易表示（バウンディングボックス表示）に切り替えて応答性を高めます。ただしこの切り替えが行われるのは、シーン中で軽量描画に対応しているモデル（例えばシーンアイテムのプロパティ「軽量レンダリング」をONにしたもの）に限られ、その部分のみが簡易表示になります。重いシーンを軽快に閲覧したい場合に有用です。
 * - FPSテスト
   - 「FPSテスト」ボタンを押すと、現在のシーンをどれくらいのフレームレートで描画できるかを計測します。試行回数も指定できます。

その他
~~~~~~

.. 英訳指示： 見出し「その他」は "Other options" としてください。
.. 英訳指示： 「対象アイテム選択用の専用のチェックをアイテムツリービューに追加」は "Use dedicated item tree view checks to select the target items" としてください。

ダイアログの末尾には「対象アイテム選択用の専用のチェックをアイテムツリービューに追加」のチェックがあります。シーンビューは複数生成して、それぞれのビューに別のオブジェクトを表示することもできます。その場合、このチェックをONにしておくとアイテムツリービューの右側に対象シーンビュー専用のチェックが表示されますので、それでシーンビューへの表示のON/OFFを行なってください。なお、シーンビューが複数ある場合、設定ダイアログの対象ビューはダイアログを表示したときに最後にフォーカスのあたっていたシーンビューとなります。
