...
この関数はメンバ変数flagで保持しているノードオブジェクトを返すようになっており、 ::
...
このノードによって、シーングラフにおける旗の位置姿勢を、BodyPositionアイテムの記録位置にあわせるようにしています。
flagはスマートポインタなので、このように生成したノードをセットしておけば、このアイテムが破棄されるまでは保持されつづけます。
またこれ以降に生成する旗の構成要素となるノードは、この最上位ノードの子ノードとして追加していきます。
