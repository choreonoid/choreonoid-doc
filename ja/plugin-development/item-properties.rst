...
これによって、例えばアイテムプロパティビュー上でこのプロパティの値を編集できるようになります。
ここでtrueが返されると、この値が実際に更新されたことがプロパティ編集側にも伝わり、値が確定して表示されます。
なお、この場合の変数idはこのプロパティを扱う間は存在しつづける必要があることにご注意ください。
...
先のラムダ式の場合と同様に、変数idはプロパティを扱う間は存在しつづける必要があります。
...
これまで説明した方法で、新たに定義したアイテム型に対してプロパティを追加できます。これに加えて、全アイテム型の基底となるItemクラスに対して、以下のようなプロパティが定義されています。これらは全アイテム型に共通のプロパティとなります。
