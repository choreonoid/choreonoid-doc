...
Ubuntu LinuxではChoreonoidの :doc:`パッケージによるインストール <install-package-ubuntu>` が可能ですが、ソースコードからビルドすることで、Choreonoid本体を開発したり、より柔軟な運用を行うことが可能となります。
ここではUbuntu Linux におけるChoreonoidのソースコードからのビルド方法について解説します。
...
展開すると choreonoid-2.3.0 といったディレクトリが生成されます。このディレクトリもソースコード一が格納された **「ソースディレクトリ」** となります。
...
C++を利用するためには、標準C++ライブラリも必要となりますが、上記のコマンドだけではclangに対応する標準C++ライブラリがインストールされないことがあります。その場合は標準C++ライブラリのパッケージもインストールするようにします。
...
などとしてインストールしておきます。
( `stack overflow の参考ページ <https://stackoverflow.com/questions/74543715/usr-bin-ld-cannot-find-lstdc-no-such-file-or-directory-on-running-flutte>`_  )
...
インストール先は、CMakeの **CMAKE_INSTALL_PREFIX** の設定で変更することが可能です。複数のアカウントで利用する必要がなければ、ホームディレクトリのどこかをインストール先にするのもよいでしょう。この場合、インストール時にsudoをする必要もなくなります。

ただし、その場合はそのままでは実行ファイルやライブラリのパスが通っていません。またプラグイン開発用のCMakeファイルなども検索対象とはなりません。これを解決するためのシェルスクリプトがインストール先にありますので、これを用いて必要な環境設定を行います。

スクリプトはインストール先の "setup.bash" というファイルになります。例えば、 "~/usr" 以下にインストールした場合は、"~/usr/setup.bash" というファイルになります。このファイルを ::

 source ~/usr/setup.bash

として取り込めばOKです。
同じ記述を ".bashrc" ファイルに含めておけば、自動でこの設定がされるようになります。
...
プラグイン等の機能面のオプションとは別に、生成するバイナリの最適化に関するCMake変数もいくつか用意されています。いずれもデフォルトはOFFで、必要に応じて有効化してください。設定方法は上記のオプション機能と同様です。

* **ENABLE_LTO** : リンク時最適化 (Link Time Optimization) を有効にします。ライブラリ境界をまたいだインライン化などが行われ、シミュレーション等の処理速度がわずかに向上します。

* **ENABLE_NATIVE_CPU_ARCHITECTURE** : コンパイラに ``-march=native`` を与え、ビルドを実行したマシンのCPUが持つ全ての命令セットを利用できるようにします。生成したバイナリはビルド時と同等以上のCPUでないと動作しないため、自分専用のビルドで最大性能を狙う場合に使用します。

* **ENABLE_X86_64_V3** : コンパイラに ``-march=x86-64-v3`` を与え、AVX2やFMA等の命令セットを利用できるようにします。おおよそ2013年以降のIntel CPU、2015年以降のAMD CPUで動作する範囲で、ポータブル性を保ったまま一定の最適化が得られます。

例えばLTOとX86_64_V3を同時に有効にする場合は、以下のようにCMakeを実行します。 ::

 cmake .. -DENABLE_LTO=ON -DENABLE_X86_64_V3=ON

なお、命令セットの拡張（ENABLE_NATIVE_CPU_ARCHITECTUREとENABLE_X86_64_V3）による高速化の効果は限定的なことが多いようです。
