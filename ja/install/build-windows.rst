...
これを利用する場合は、 `Git for Windows 公式サイト <https://gitforwindows.org/>`_ からファイルをダウンロードしてインストールを行ってください。インストール時の設定はデフォルトのままでよいと思いますが、途中下図の様に表示されたら"Use Git from the Windows Command Prompt"を選ぶと、コマンドプロンプトからこのコマンドを実行するためのPATHを通してくれます。
