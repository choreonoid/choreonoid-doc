...
.. note:: 上記のコマンドの例では、インストール先のディレクトリの区切りに "/" （スラッシュ）を使用しています。Windowsでは"\" (バッククォート又は¥記号）が標準の区切り文字となりますが、CMakeのオプションについては、他のOSと同様にスラッシュも利用可能です。逆にバッククォートを使用すると、Git for Bashのコマンドラインでディレクトリが正しく認識されない場合があります。そのような環境ではバッククォートではなくスラッシュを使うようにしてください。
