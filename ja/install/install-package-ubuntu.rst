...
上記の開発版とリリース版のリポジトリは、両方同時に登録しておくこともできます。その場合、 ``sudo apt install choreonoid`` を実行すると、ふたつのリポジトリのうちバージョン番号がより新しいパッケージがインストールされます。通常は開発版のほうがリリース版より新しいバージョン番号を持つので、開発版がインストールされることになります。

もし両方のリポジトリを登録した上で、明示的にリリース版や開発版を指定してインストールしたい場合は、以下のように行います。
...
.. code-block:: text
   ...
   choreonoid:
     インストールされているバージョン: (なし)
     候補:               2.5.0~git20260514.2131.8ad62ee1b-1~noble
     バージョンテーブル:
...
「バージョンテーブル」には、登録されているリポジトリにある最新のパッケージがそれぞれ表示されています。"2.5.0~git20260514.2131.8ad62ee1b-1~noble" が testing(開発版)リポジトリにあるパッケージで、"2.4.0-1~noble" が stable(リリース版)リポジトリにあるパッケージです。

これらの中から、特定のものをインストールしたい場合は、 ::

 sudo apt install choreonoid=2.4.0-1~noble

のようにバージョン名を指定してインストールします。上の例では、開発版とリリース版の両方が登録されている状態で、安定性を重視してリリース版(2.4.0)を明示的にインストールしています。
